	...
	<div class="container1">
		<div class="row">
			...
			<div class="col-lg-offset-2 main">
				<h3 class="page-header">私海客户信息</h3>
				<div class="row head">
				...
				<div class="col-md-8" style="width: 40%;float: left;">
					...
					<div style="clear: both;" class="col-md-8">
					<h4>当前跟进信息</h4>
					<table class="table table-striped table-bordered " style="margin-bottom: 0px;">
						<tr>
							<th>职务</th><th>姓名</th><th>手机</th><th>微信</th>
						</tr>
						<tr>
							<td>
								<c:choose>
									<c:when test="${contact.duty==1}">老板</c:when>
									<c:when test="${contact.duty==2}">店长</c:when>
									<c:when test="${contact.duty==3}">前台</c:when>
									<c:when test="${contact.duty==4}">服务员</c:when>
									<c:when test="${contact.duty==5}">其他</c:when>
								</c:choose>
							</td>
							<td>${contact.name}</td><td>${contact.telephone}</td><td>${contact.wechat}</td>
						</tr>
					</table>
					</div>
				</div>
